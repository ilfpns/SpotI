import { DEFAULT_LABEL_COLOR, DEFAULT_CASE_COLOR, DEFAULT_SHOW_BORDER } from "./theme";
 ...
 * on-screen pet design. The LP's center label, the sleeve/case, and whether
 * either shows its outline stroke are all user-themeable — see
 * shared/theme.ts — everything else is fixed.
 */
...
<svg id="pet-svg" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg">
  ...
  <circle id="pet-disc-border" cx="39" cy="24" r="21" fill="none" stroke="#d8d6d0" stroke-width="${borderWidth}" />
  ...
  <rect id="pet-case" x="3" y="3" width="36" height="42" rx="2" fill="${caseColor}" stroke="#d8d6d0" stroke-width="${borderWidth}" />
</svg>
`.trim();
}
