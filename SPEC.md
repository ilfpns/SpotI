...
- **클릭 시 케이스가 슬라이드되어 LP가 전부 드러남**(다시 클릭하면
  원위치). 벗겨질 때는 오른쪽 아래 대각선으로 슬라이드되어 사라지고,
  다시 씌울 때는 같은 경로를 거슬러 왼쪽 위로 올라와 LP와 포개어짐(같은
  CSS 전환의 양방향이라 자연스럽게 대칭됨). 슬라이드 속도는 설정에서
  조절 가능(애니메이션 탭, 4.4절 참고).
...
- **케이스 모양**: 두 가지 중 선택.
  ...
  - 두 모양 모두 `shared/petSvg.ts`의 `casePathFor()` 하나가 만들어내는
    ...
    애니메이션은 오른쪽·아래로 각각 70px씩(대각선) 이동하는데, 두 모양
    중 더 넓은 쪽(`cut`, 오른쪽 끝 x=60 / `classic`, 왼쪽 끝 x=-3) 기준
    가로 이동만으로도 이미 화면 밖으로 완전히 빠져나가는 여유값이라
    어느 모양이든 확실히 사라짐.
