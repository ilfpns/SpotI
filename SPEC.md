...
| 연동(폴링) 속도 | `appSettingsStore` | fast(200ms) — 팝업 열려 있을 때 이 값, 닫혀있으면 3배 느긋하게(7절 참고) |
...
- **폴링**: 설정의 폴링 속도(fast=200ms/normal=1000ms)를 기준으로,
  팝업이 실제로 열려 있을 때만 그 값 그대로, 닫혀있을 때(유휴 —
  펫이 대부분의 시간을 보내는 상태)는 그 값의 3배(`IDLE_MULTIPLIER`)로
  느긋하게 폴링함. `startPolling()`/`setPollingActive()` 양쪽 모두
  `applyInterval()`을 거쳐 항상 저장된 속도 설정을 반영.
  - 원래는 유휴 시 속도 설정과 무관하게 고정 3000ms를 썼음 — "빠름"을
    골라도 체감이 거의 안 됨 → 한 번 "유휴/활성 구분 없이 항상 설정값
    그대로" 폴링하도록 바꿈("연동 속도를 최대로" 요청) → 이번엔
    자기 자신의 액션에 대한 반응성은 폴링 간격과 무관하게 이미
    낙관적 방송(`broadcastOptimisticPlayState`)으로 즉시 반영되고
    있었는데도, 평소 사용 중에도 초당 5번(200ms)씩 계속 요청이
    나가면서 실제 Spotify 레이트리밋을 (제가 오늘 테스트하며 여러 번
    걸렸던 것처럼) 일상적인 사용만으로도 건드릴 수 있다는 우려가
    나와서 — 지금의 "유휴는 3배로 느긋하게, 그래도 설정값 기준"
    절충안으로 정착. 429 레이트리밋 응답 시 로컬에서 일정 시간
    요청을 멈추는 회로차단기(circuit breaker)는 여전히 적용.
