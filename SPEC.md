...
- LP 상단에 사용자가 지정한 이름을 표시 가능(케이스 탭, 4.3절 참고).
...
- LP 이름은 영문·숫자·공백·일부 기호(``!?.,'"&#@_-``)만 허용, 최대
  ...
  (`shared/theme.ts`의 `sanitizeDiscName`)로 검증. LP 상단, 라벨 원
  위쪽 어두운 영역에 표시되며 디스크와 함께 회전함.
...
화면 모드=다크, 테두리 표시=켜짐, LP 이름=빈 값, 크기=medium,
